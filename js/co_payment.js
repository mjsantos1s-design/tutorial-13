...
function validateCVC() {
   ...
   if (cardCVC.validate.valueMissing) {
      cardCVC.setCustomValidity("Enter your CVC number");
   } else if ((creditCard === "amex") && (/^\d{4}$/.test(creditCVC.value) === false)) {
      cardCVC.setCustomValidity("Enter a 4-digit CVC number");
   } else if ((creditCard === "amex") && (/^\d{3}$/.test(creditCVC.value) === false)) {
...
function luhn(idNum) {
   ...
   //Retireve the even-numbered digits and double them
